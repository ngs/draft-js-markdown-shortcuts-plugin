...
describe('adjustBlockDepth', () => {
  ...
  describe('non list item', () => {
    it('does not add depth', () => {
      ...
      expect(Draft.convertToRaw(newEditorState.getCurrentContent())).to.deep.equal(rawContentState('unstyled', 0, 0));
    });
  });
  ['unordered-list-item', 'ordered-list-item', 'checkable-list-item'].forEach(type => {
    describe(type, () => {
      it('adds depth', () => {
        ...
        expect(Draft.convertToRaw(newEditorState.getCurrentContent())).to.deep.equal(rawContentState(type, 0, 1));
      });
    });
  });
});
